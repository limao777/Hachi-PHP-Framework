...
class Model_PDOSqlMaker 
{
    ...
    public function where($where, $attrs = array())
    {
        ...
        $bind_attrs = array();
        if (!empty($where)) {
            $whereSql = $this->_where($where, $bind_attrs);
            if ( $whereSql) {
                $sql .= ' WHERE ' . $whereSql;    
            }
        }
    ...
    private function _where($where, &$bind_attrs)
    {
        ...
        foreach ($where as $key => $val) {
            $conds[] = $this->_cond($key, $val, $bind_attrs);
        }
        if ( ! $logic) {
            $logic = 'AND';
        }
    ...
    private function _cond($name, $val, &$bind_attrs)
    {
